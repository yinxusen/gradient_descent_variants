 ...
 */
@DeveloperApi
class GradientDescentAnother(private var gradient: Gradient, private var updater: Updater)
  ...
  override def optimize(data: RDD[(Double, Vector)], initialWeights: Vector): Vector = {
    val (weights, _) = GradientDescentAnother.runMiniBatchSGD(
      data,
      gradient,
      updater,
      stepSize,
 ...
 */
@DeveloperApi
object GradientDescentAnother extends Logging {
  /**
  ...
  def runMiniBatchSGD(
    ...
    for (i <- 1 to numIterations) {
      ...
      val (gradientSum, lossSum) = data.sample(false, miniBatchFraction, 42 + i)
        .aggregate((BSV.zeros[Double](weights.size), 0.0))(
